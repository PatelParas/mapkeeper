...
public:
    RocksDbServer(const std::string& directoryName,
        ...
        cache_ = rocksdb::NewLRUCache(blockCacheSizeMb_ * 1024 * 1024);
        ...
        Options rocksdb_options = ConvertOptions(options);  
        //rocksdb_options.block_cache = cache_;
        ...
        for (directory_iterator itr(directoryName); itr != end_itr;itr++) {
            if (is_directory(itr->status())) {
                ...
                Status status = DB::Open(rocksdb_options, itr->path().string(), &db);
                assert(status.ok());
    ...
    ResponseCode::type addMap(const std::string& mapName) {
        ...
        // options.block_cache = cache_;
        Options rocksdb_options = ConvertOptions(options);
        Status status = DB::Open(rocksdb_options, directoryName_ + "/" + mapName, &db);
    ...
    ResponseCode::type dropMap(const std::string& mapName) {
        ...
        maps_.erase(itr);
        DestroyDB(directoryName_ + "/" + mapName, Options());
        return ResponseCode::Success;
    }
    ...
    void scanAscending(RecordListResponse& _return, DB* db, 
              const std::string& startKey, const bool startKeyIncluded, 
              const std::string& endKey, const bool endKeyIncluded,
              const int32_t maxRecords, const int32_t maxBytes) {
    ...
    void scanDescending(RecordListResponse& _return, DB* db,
              const std::string& startKey, const bool startKeyIncluded, 
              const std::string& endKey, const bool endKeyIncluded,
              const int32_t maxRecords, const int32_t maxBytes) {
        int numBytes = 0;
    ...
    void get(BinaryResponse& _return, const std::string& mapName, const std::string& key) {
        ...
        Status status = itr->second->Get(ReadOptions(), key, &(_return.value));
    ...
    ResponseCode::type put(const std::string& mapName, const std::string& key, const std::string& value) {
        ...
        Status status = itr->second->Put(options, key, value);
	...
	if(!blindinsert) {
	  ...
	  Status status = itr->second->Get(ReadOptions(), key, &recordValue);
	  if (status.ok()) {
    ...
    ResponseCode::type remove(const std::string& mapName, const std::string& key) {
        ...
        WriteOptions options;
        options.sync = true;
        Status status = itr->second->Delete(options, key);
...
int main(int argc, char **argv) {
    ...
    config.add_options()
        ...
        ("write-buffer-mb,w", po::value<int>(&writeBufferSizeMb)->default_value(1024), "RocksDB write buffer size in MB")
        ("block-cache-mb,b", po::value<int>(&blockCacheSizeMb)->default_value(1024), "RocksDB block cache size in MB")
        ;
